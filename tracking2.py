# -*- coding: utf-8 -*-
"""
Created on Tue Sep  8 11:30:55 2020
...
AOV_row = 70
AOV_col = 60
pix_angle_row = np.linspace(-AOV_row/2, AOV_row/2, 1920+1)
pix_angle_row = pix_angle_row[0:1920]
pix_angle_row = np.tile(pix_angle_row, (1080,1))
pix_angle_col = np.linspace(-AOV_col/2, AOV_col/2, 1080+1)
pix_angle_col = pix_angle_col[0:1080]
pix_angle_col = np.tile(pix_angle_col, (1920,1))
pix_angle_col = np.transpose(pix_angle_col)
focal_length = 1081.37*1e-3
...
while True:
    ...
    if target_shape == 'circle':
        ...
        for cnt in contours: 
            ...
            if area > 20000:  #stick to 3000 for now
                approx = cv.approxPolyDP(cnt,  
                                     0.03 * cv.arcLength(cnt, True), True) #dont change 0.05
                ...
                if found:
                    ...
                    real_area = 0                             
                    for idx in range(len(pixelpoints)-1):   
                        p1 = pixelpoints[idx]
                        p2 = pixelpoints[idx+1]
                        
                        if depth[p1[0]][p1[1]] == 0:
                            d1 = depth_mean
                        else:
                            d1 = depth[p1[0]][p1[1]] 
                                      
                        if depth[p2[0]][p2[1]] == 0:
                            d2 = depth_mean
                        else:
                            d2 = depth[p1[0]][p1[1]]              
                        
                
                        w1 = abs(d1*np.tan(pix_angle_row[p1[0]][p1[1]]*np.pi/180))
                        w2 = abs(d2*np.tan(pix_angle_row[p2[0]][p2[1]]*np.pi/180))
                        h1 = abs(d1*np.tan(pix_angle_col[p1[0]][p1[1]]*np.pi/180))
                        h2 = abs(d2*np.tan(pix_angle_col[p2[0]][p2[1]]*np.pi/180))
                        
                        real_area += (w2-w1)*(h2-h1)
